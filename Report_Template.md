...
| Task B | 邓淑婧20241050232 | Stella10009 |  |
...
是，已通过检查。
1.偶数分段检查：在 simpson_composite 函数中，我加入了 if n % 2 != 0: n += 1 的逻辑。这确保了即使用户输入奇数，程序也能自动修正为偶数，满足 Simpson 法则对区间成对分割的数学要求。
2.边界与奇点检查：在 debye_integrand 函数中，我加入了 if abs(x) < 1e-12: return 0.0 的判断。这有效处理了 x→0 时的 0/0 型奇点（利用极限可知值为0），避免了程序报错。

- 同一参数下方法比较：

| 方法 | n | 积分值 | 误差估计 | 结论 |
| 梯形法 | 50 | 0.876302 | 较高 (基准) | 收敛速度较慢，需要更细分的区间才能达到高精度 |
| Simpson 法 | 50 | 0.876165 | 较低 (更精确) | 精度显著更高，在相同计算量下更接近真实值 |

- 对 Debye 积分结果的解释：  
1.低温 T^3 律的完美验证：
从物理规律验证部分可以看到，在 T=10K （远小于 θD=428K ）时，数值积分结果为 25.97576，与理论极限值 (4π)^4/15≈25.97576 的偏差仅为 9.48×10^−13 。这极好地证明了代码在低温极限下的正确性，符合 Debye 模型关于热容 Cv∝T^3的预测。
2.数值方法的精度对比：
对比表格数据可见，Simpson 法在所有温度下都表现出比梯形法更高的稳定性。特别是T=300K和 500K 这种高温区（此时积分上限 y 较小，函数变化相对剧烈），Simpson 法利用二次抛物线拟合被积函数，其 O(h^4)的收敛速度明显优于梯形法的 O(h^2)
3.温度的影响：
随着温度从 10K 升高到 500K，积分值从约 26 迅速下降到 0.2。这反映了物理事实：在低温下，更多的声子模式被“冻结”，积分上限 y=θD/T 很大，积分贡献大；而在高温下，积分上限变小，被积函数区间缩短，积分值随之减小，热容趋于经典常数。
---
